	...
	var dhxCustomScroll = dhtmlx.CustomScroll = {
		...
		enable:function(view, mode){ 
			...
			if (view && view.attachEvent){
				view.attachEvent("onOpenEnd", function(){
					if (node._mouse_inside){
						dhxCustomScroll._mouse_out_timed.call(node);
						dhxCustomScroll._mouse_in.call(node);
					}
				})
				view.attachEvent("onSubGridLoaded", function(){
					if (node._mouse_inside){
						dhxCustomScroll._mouse_out_timed.call(node);
						dhxCustomScroll._mouse_in.call(node);
					}
				})
				view.attachEvent("onSubRowOpen", function(){
					if (node._mouse_inside){
						dhxCustomScroll._mouse_out_timed.call(node);
						dhxCustomScroll._mouse_in.call(node);
					}
				})
				view.attachEvent("onSubAjaxLoad", function(){
					if (node._mouse_inside){
						dhxCustomScroll._mouse_out_timed.call(node);
						dhxCustomScroll._mouse_in.call(node);
					}
		...
		_mouse_wheel:function(e){
			if (this.grid.entBox.className=="dhx_sub_row")
				that==this.parentNode
			else
			that=this
			var sizes = that._custom_scroll_size;
			...
			if (sizes){
				if (sizes._scroll_x_node && (e.wheelDeltaX || ( delta && !sizes._scroll_y_node ))){
					...
					//see below
					dhxCustomScroll._set_scroll_value(that, "scrollLeft", x_dir*dhxCustomScroll.scrollStep);
					...
					dhxCustomScroll._set_scroll_value(that, "scrollTop", delta*dhxCustomScroll.scrollStep);
				}
			}

			
			dhxCustomScroll._update_scroll(that);
